...
const { filterAspects } = require("./astroUtils");
...
class AstrologicalCalculator {
  ...
  generateChart(birthData) {
    ...
    aspects = filterAspects(
      aspects,
      { ...planets, ...angles },
      this.considerAspectsCompatibility,
    );
  ...
  calculateSolarReturn(solarReturnData, natalChart) {
    const { birthDate, birthPlace, returnYear, currentLocation } =
      solarReturnData;
    ...
    try {
      ...
      const solarReturnMoment = this.findSolarReturnMoment(
        natalSunLongitude,
        returnYear,
        birthDate,
      );
      ...
      solarReturnChart.planets = this.addHouse(
        solarReturnChart.planets,
        natalChart.houses,
        "natal_house",
      );
      solarReturnChart.angles = this.addHouse(
      ...
      return {
        ...
        meta: {
          ...
          solar_return: {
            ...
            same_location:
              JSON.stringify(location) === JSON.stringify(birthPlace),
          },
  ...
  addHouse(positions, houses, fieldName = "house") {
    Object.keys(positions).forEach((key) => {
      positions[key][fieldName] = this.determinePlanetHouse(
        positions[key].longitude,
        houses,
      ).number;
    });
  ...
  calculatePlanetaryPositions(julianDay, houses = null) {
    ...
    for (const [planetName, planetId] of Object.entries(this.planets)) {
      try {
        ...
        if (result.rflag >= 0) {
          ...
          if (houses) {
            planetData.house = this.determinePlanetHouse(
              result.longitude,
              houses,
            ).number;
          }
        ...
        console.warn(
          `Error calculating position for ${planetName}:`,
          error.message,
        );
      }
    }
    ...
    if (positions.NNode) {
      ...
      if (southNode && houses) {
        southNode.house = this.determinePlanetHouse(
          southNode.longitude,
          houses,
        ).number;
      }
        ...
        if (aspect) {
          if (this.filterAspect(planet1, planet2, aspect)) {
            aspects.push({
              ...
              applying: this.isNatalAspectApplying(
                positions[planet1],
                positions[planet2],
                aspect.angle,
              ),
  ...
  calculateAspectsOfTwoCharts(
    ...
    for (const [transit_planet, transit_position] of Object.entries(
      transitPositions,
    )) {
      // Transit/Partner
      for (const [natal_planet, natal_position] of Object.entries(
        natalPositions,
      )) {
        // Natal
        ...
        if (aspectData) {
          ...
          aspect[index1] = {
            ...
            sign: transit_position.sign,
            house: transit_position.house,
            longitude: transit_position.longitude,
          };
          aspect[index2] = {
            planet: natal_planet,
            sign: natal_position.sign,
            house: natal_position.house,
            longitude: natal_position.longitude,
          };
          ...
          if (isTransit) {
            ...
            aspect.applying = this.isTransitApplying(
              transit_position,
              natal_position,
              aspectData.targetAngle,
            );
            aspect.peakDate = this.estimatePeakDate(
              transit_position,
              natal_position,
              aspectData.targetAngle,
              transitDate,
  ...
  findTransitAspect(transitLong, natalLong, transitPlanet) {
    ...
    for (const [aspectName, aspectData] of Object.entries(
      this.transitAspects,
    )) {
  ...
  getTransitOrb(transitPlanet, aspectName) {
    const planetOrbs = this.transitOrbs[transitPlanet] || {
      major: 1,
      minor: 0.5,
    };
    return this.majorAspects.includes(aspectName)
      ? planetOrbs.major
      : planetOrbs.minor;
  }
  ...
  isTransitApplying(transitPos, natalPos, targetAngle) {
    ...
    return (
      Math.abs(futureAngle - targetAngle) < Math.abs(currentAngle - targetAngle)
    );
  }
  ...
  isNatalAspectApplying(planet1, planet2, aspectAngle) {
    ...
    return (
      Math.abs(futureAngle - aspectAngle) < Math.abs(currentAngle - aspectAngle)
    );
  }
