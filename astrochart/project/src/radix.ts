...
class Radix {
  ...
  constructor(
    ...
    this.rulerRadius =
      this.radius /
      this.settings.INNER_CIRCLE_RADIUS_RATIO /
      this.settings.RULER_RADIUS;
    this.pointRadius =
      this.radius -
      (this.radius / this.settings.INNER_CIRCLE_RADIUS_RATIO +
        2 * this.rulerRadius +
        this.settings.PADDING * this.settings.SYMBOL_SCALE);
    ...
    const divisionForAspects = document.createElementNS(
      this.paper.root.namespaceURI,
      "g",
    );
    divisionForAspects.setAttribute(
      "id",
      this.paper.root.id + "-" + this.settings.ID_ASPECTS,
    );
    ...
    this.universe.setAttribute(
      "id",
      this.paper.root.id + "-" + this.settings.ID_RADIX,
    );
  ...
  drawBg(): void {
    ...
    const wrapper = getEmptyWrapper(
      universe,
      this.paper.root.id + "-" + this.settings.ID_BG,
      this.paper.root.id,
    );
    ...
    const hemisphere = this.paper.segment(
      this.cx,
      this.cy,
      this.radius - this.radius / this.settings.INNER_CIRCLE_RADIUS_RATIO,
      start,
      end,
      this.radius / this.settings.INDOOR_CIRCLE_RADIUS_RATIO,
      LARGE_ARC_FLAG,
    );
    hemisphere.setAttribute(
      "fill",
      this.settings.STROKE_ONLY ? "none" : this.settings.COLOR_BACKGROUND,
    );
  ...
  drawUniverse(): void {
    ...
    for (
      let i = 0,
        step = 30,
        start = this.shift,
        len = this.settings.COLORS_SIGNS.length;
      i < len;
      i++
    ) {
      const segment = this.paper.segment(
        this.cx,
        this.cy,
        this.radius,
        start,
      ...
      segment.setAttribute(
        "fill",
        this.settings.STROKE_ONLY ? "none" : this.settings.COLORS_SIGNS[i],
      );
      segment.setAttribute(
        "id",
        this.paper.root.id +
      ...
      segment.setAttribute(
        "stroke",
        this.settings.STROKE_ONLY ? this.settings.CIRCLE_COLOR : "none",
      );
      segment.setAttribute(
        "stroke-width",
        this.settings.STROKE_ONLY ? "1" : "0",
      );
    ...
    for (
      let i = 0,
        step = 30,
        start = 15 + this.shift,
        len = this.settings.SYMBOL_SIGNS.length;
      i < len;
      i++
    ) {
      const position = getPointPosition(
        this.cx,
        this.cy,
        this.radius - this.radius / this.settings.INNER_CIRCLE_RADIUS_RATIO / 2,
        start,
        this.settings,
      );
      wrapper.appendChild(
        this.paper.getSymbol(
          this.settings.SYMBOL_SIGNS[i],
          position.x,
          position.y,
        ),
      );
  ...
  drawPoints(): void {
    ...
    const gap =
      ...
      (this.radius / this.settings.INNER_CIRCLE_RADIUS_RATIO +
        this.radius / this.settings.INDOOR_CIRCLE_RADIUS_RATIO);
    const step =
      (gap - 2 * (this.settings.PADDING * this.settings.SYMBOL_SCALE)) /
    ...
    for (const planet in this.data.planets) {
      if (this.data.planets.hasOwnProperty(planet)) {
        ...
        this.locatedPoints = assemble(
          this.locatedPoints,
          point,
          { cx: this.cx, cy: this.cy, r: this.pointRadius },
          this.settings,
        );
      }
    }
    ...
    this.locatedPoints.forEach(function (point: LocatedPoint) {
      // draw pointer
      startPosition = getPointPosition(
        this.cx,
        this.cy,
        pointerRadius,
      ...
      pointer.setAttribute(
        "stroke-width",
        this.settings.CUSPS_STROKE * this.settings.SYMBOL_SCALE,
      );
      wrapper.appendChild(pointer);

      // draw pointer line
      if (
        !this.settings.STROKE_ONLY &&
        ...
        endPosition = getPointPosition(
          ...
          this.cy,
          this.pointRadius +
            this.settings.COLLISION_RADIUS * this.settings.SYMBOL_SCALE,
          point.angle,
          this.settings,
        );
        const line = this.paper.line(
          startPosition.x,
          startPosition.y,
          endPosition.x,
          endPosition.y,
        ...
        line.setAttribute(
          "stroke-width",
          0.5 * (this.settings.CUSPS_STROKE * this.settings.SYMBOL_SCALE),
        );
      ...
      let textsToShow = [
        (Math.floor(this.data.planets[point.name][0]) % 30).toString(),
      ];

      const zodiac = new Zodiac(this.data.cusps, this.settings);

      if (
        this.data.planets[point.name][1] &&
        zodiac.isRetrograde(this.data.planets[point.name][1])
      ) {
        textsToShow.push("R");
      } else {
        textsToShow.push("");
      ...
      if (this.settings.SHOW_DIGNITIES_TEXT)
        textsToShow = textsToShow.concat(
          zodiac
            .getDignities(
              { name: point.name, position: this.data.planets[point.name][0] },
              this.settings.DIGNITIES_EXACT_EXALTATION_DEFAULT,
            )
            .join(","),
        );

      const pointDescriptions = getDescriptionPosition(
        point,
        textsToShow,
        this.settings,
      );
  ...
  drawAxis(): void {
    ...
    const wrapper = getEmptyWrapper(
      ...
      this.paper.root.id +
        ...
        this.settings.ID_RADIX +
        "-" +
        this.settings.ID_AXIS,
      this.paper.root.id,
    );

    const axisRadius =
      this.radius + this.radius / this.settings.INNER_CIRCLE_RADIUS_RATIO / 4;
    ...
    [AS, IC, DC, MC].forEach(function (i) {
      ...
      // As
      if (i === AS && this.settings.SHOW_AXIS_LABELS) {
        // Text
        textPosition = getPointPosition(
          this.cx,
          this.cy,
          axisRadius + 20 * this.settings.SYMBOL_SCALE,
          this.data.cusps[i] + this.shift,
          this.settings,
        );
        wrapper.appendChild(
          this.paper.getSymbol(
            this.settings.SYMBOL_AS,
            textPosition.x,
            textPosition.y,
          ),
        );
      ...
      // Ds
      if (i === DC && this.settings.SHOW_AXIS_LABELS) {
        // Text
        textPosition = getPointPosition(
          this.cx,
          this.cy,
          axisRadius + 2 * this.settings.SYMBOL_SCALE,
          this.data.cusps[i] + this.shift,
          this.settings,
        );
        wrapper.appendChild(
          this.paper.getSymbol(
            this.settings.SYMBOL_DS,
            textPosition.x,
            textPosition.y,
          ),
        );
      ...
      // Ic
      if (i === IC && this.settings.SHOW_AXIS_LABELS) {
        // Text
        textPosition = getPointPosition(
          this.cx,
          this.cy,
          axisRadius + 10 * this.settings.SYMBOL_SCALE,
          this.data.cusps[i] - 2 + this.shift,
          this.settings,
        );
        wrapper.appendChild(
          this.paper.getSymbol(
            this.settings.SYMBOL_IC,
            textPosition.x,
            textPosition.y,
          ),
        );
      ...
      // Mc
      if (i === MC && this.settings.SHOW_AXIS_LABELS) {
        // Text
        textPosition = getPointPosition(
          this.cx,
          this.cy,
  ...
  aspects(customAspects?: FormedAspect[] | null): Radix {
    const aspectsList =
      customAspects != null && Array.isArray(customAspects)
        ? customAspects
        : new AspectCalculator(this.toPoints, this.settings).radix(
            this.data.planets,
          );
    ...
    for (let i = 0, ln = aspectsList.length; i < ln; i++) {
      ...
      if (duplicateCheck.indexOf(opositeKey) === -1) {
        ...
        const startPoint = getPointPosition(
          this.cx,
          this.cy,
          this.radius / this.settings.INDOOR_CIRCLE_RADIUS_RATIO,
        ...
        const endPoint = getPointPosition(
          this.cx,
          this.cy,
          this.radius / this.settings.INDOOR_CIRCLE_RADIUS_RATIO,
  ...
  drawRuler(): void {
    ...
    const startRadius =
      ...
      (this.radius / this.settings.INNER_CIRCLE_RADIUS_RATIO +
        this.rulerRadius);
    const rays = getRulerPositions(
      this.cx,
      this.cy,
      startRadius,
      startRadius + this.rulerRadius,
      this.shift,
      this.settings,
    );
  ...
  drawCircles(): void {
    ...
    circle.setAttribute(
      "stroke-width",
      (this.settings.CIRCLE_STRONG * this.settings.SYMBOL_SCALE).toString(),
    );
    wrapper.appendChild(circle);
